...
describe("LD B, C test", () => {
  ...
  test("clone", () => {
    const instruction = new LdBC(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdBC);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdBC(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    // B レジスタの内容が変わっていることの確認
    expect(register.B).toBe(prevRegister.C);
    expect(register.BC).toBe(0x4444);
