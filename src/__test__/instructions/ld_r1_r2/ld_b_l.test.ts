...
describe("LD B, L test", () => {
  ...
  test("clone", () => {
    const instruction = new LdBL(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdBL);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdBL(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    expect(register.B).toBe(prevRegister.L);
    expect(register.BC).toBe(0x8844);
