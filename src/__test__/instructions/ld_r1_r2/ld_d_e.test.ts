...
describe("LD D, E test", () => {
  ...
  test("clone", () => {
    const instruction = new LdDE(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdDE);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdDE(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    expect(register.D).toBe(prevRegister.E);
    expect(register.DE).toBe(0x6666);
