...
describe("LD E, B test", () => {
  ...
  test("clone", () => {
    const instruction = new LdEB(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdEB);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdEB(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    expect(register.E).toBe(prevRegister.B);
    expect(register.DE).toBe(0x5533);
