...
describe("LD A, L test", () => {
  ...
  test("clone", () => {
    const instruction = new LdAL(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdAL);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdAL(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    expect(register.A).toBe(prevRegister.L);
    expect(register.AF).toBe(0x8822);
