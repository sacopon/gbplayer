...
describe("LD D, H test", () => {
  ...
  test("clone", () => {
    const instruction = new LdDH(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdDH);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdDH(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    expect(register.D).toBe(prevRegister.H);
    expect(register.DE).toBe(0x7766);
