...
describe("LD C, C test", () => {
  ...
  test("clone", () => {
    const instruction = new LdCC(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdCC);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdCC(new CpuOperation(register, memory));
    instruction.fetch();
