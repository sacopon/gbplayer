...
describe("LD A, H test", () => {
  ...
  test("clone", () => {
    const instruction = new LdAH(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdAH);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdAH(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    expect(register.A).toBe(prevRegister.H);
    expect(register.AF).toBe(0x7722);
