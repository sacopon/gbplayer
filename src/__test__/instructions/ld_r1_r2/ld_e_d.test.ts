...
describe("LD E, D test", () => {
  ...
  test("clone", () => {
    const instruction = new LdED(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdED);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdED(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    expect(register.E).toBe(prevRegister.D);
    expect(register.DE).toBe(0x5555);
