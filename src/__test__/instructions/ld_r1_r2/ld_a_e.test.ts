...
describe("LD A, E test", () => {
  ...
  test("clone", () => {
    const instruction = new LdAE(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdAE);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdAE(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    expect(register.A).toBe(prevRegister.E);
    expect(register.AF).toBe(0x6622);
