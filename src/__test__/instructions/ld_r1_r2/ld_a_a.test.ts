...
describe("LD A, A test", () => {
  ...
  test("clone", () => {
    const instruction = new LdAA(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdAA);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdAA(new CpuOperation(register, memory));
    instruction.fetch();
