...
describe("LD E, L test", () => {
  ...
  test("clone", () => {
    const instruction = new LdEL(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdEL);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdEL(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    expect(register.E).toBe(prevRegister.L);
    expect(register.DE).toBe(0x5588);
