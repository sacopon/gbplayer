...
describe("LD C, L test", () => {
  ...
  test("clone", () => {
    const instruction = new LdCL(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdCL);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdCL(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    expect(register.C).toBe(prevRegister.L);
    expect(register.BC).toBe(0x3388);
