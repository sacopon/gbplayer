...
describe("LD C, B test", () => {
  ...
  test("clone", () => {
    const instruction = new LdCB(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdCB);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdCB(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    expect(register.C).toBe(prevRegister.B);
    expect(register.BC).toBe(0x3333);
