...
describe("LD D, B test", () => {
  ...
  test("clone", () => {
    const instruction = new LdDB(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdDB);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdDB(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    expect(register.D).toBe(prevRegister.B);
    expect(register.DE).toBe(0x3366);
