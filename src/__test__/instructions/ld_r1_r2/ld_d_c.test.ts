...
describe("LD D, C test", () => {
  ...
  test("clone", () => {
    const instruction = new LdDC(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdDC);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdDC(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    expect(register.D).toBe(prevRegister.C);
    expect(register.DE).toBe(0x4466);
