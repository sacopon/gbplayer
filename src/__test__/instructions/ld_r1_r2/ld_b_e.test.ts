...
describe("LD B, E test", () => {
  ...
  test("clone", () => {
    const instruction = new LdBE(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdBE);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdBE(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    expect(register.B).toBe(prevRegister.E);
    expect(register.BC).toBe(0x6644);
