...
describe("LD A, D test", () => {
  ...
  test("clone", () => {
    const instruction = new LdAD(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdAD);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdAD(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    expect(register.A).toBe(prevRegister.D);
    expect(register.AF).toBe(0x5522);
