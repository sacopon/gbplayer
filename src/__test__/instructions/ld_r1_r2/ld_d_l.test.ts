...
describe("LD D, L test", () => {
  ...
  test("clone", () => {
    const instruction = new LdDL(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdDL);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdDL(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    expect(register.D).toBe(prevRegister.L);
    expect(register.DE).toBe(0x8866);
