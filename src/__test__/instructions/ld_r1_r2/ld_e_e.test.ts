...
describe("LD E, E test", () => {
  ...
  test("clone", () => {
    const instruction = new LdEE(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdEE);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdEE(new CpuOperation(register, memory));
    instruction.fetch();
