...
describe("LD B, D test", () => {
  ...
  test("clone", () => {
    const instruction = new LdBD(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdBD);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdBD(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    expect(register.B).toBe(prevRegister.D);
    expect(register.BC).toBe(0x5544);
