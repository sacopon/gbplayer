...
describe("LD E, H test", () => {
  ...
  test("clone", () => {
    const instruction = new LdEH(new CpuOperation(register, new Memory(new Uint8Array(buffer))));
    const cloned = instruction.clone();

    expect(cloned).toBeInstanceOf(LdEH);
  });
  test("exec", () => {
    // レジスタにテスト用の初期値を設定
    register.AF = 0x1122;
    ...
    const instruction = new LdEH(new CpuOperation(register, memory));
    instruction.fetch();
    ...
    expect(register.E).toBe(prevRegister.H);
    expect(register.DE).toBe(0x5577);
